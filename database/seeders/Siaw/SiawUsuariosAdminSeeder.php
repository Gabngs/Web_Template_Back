...
/**
 * Registra el content_model 'usuarios' y sus 4 permisos estándar — sin esto
 * no hay forma de gatear fino el CRUD de "Control de usuarios" ni el menú
 * que lo desbloquea en el sidebar.
 */
class SiawUsuariosAdminSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if (!$model) {
            $modelPkid = DB::connection($connection)->table('siaw_content_model')->insertGetId([
                ...
                'app_label'      => 'siaw',
                'app_model'      => 'usuarios',
                'nombre_display' => 'Usuarios del sistema',
                'created_at'     => $now,
                'updated_at'     => $now,
            ]);
        } else {
        ...
        $permisos = [
            ['codename' => 'can_view_usuarios',   'desc' => 'Puede visualizar la lista de usuarios'],
            ['codename' => 'can_create_usuarios', 'desc' => 'Puede crear nuevos usuarios'],
            ['codename' => 'can_update_usuarios', 'desc' => 'Puede modificar usuarios existentes'],
            ['codename' => 'can_delete_usuarios', 'desc' => 'Puede eliminar usuarios'],
        ];
